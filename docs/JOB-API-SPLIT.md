...
| `fec-finance-summary` | none — `scripts/run-fec-finance-summary.ts` by hand only until 2026-09-25 (added then) | SECOND step of `ev-jobs-fec-burst` (`0 6 * * *`), after `fec-auto-match`, before `fec-burst` — **LIGHT (capped)**: stalest first (no summary, then oldest `finance_summary->>'refreshed_at'`), at most `FEC_FINANCE_SUMMARY_MAX_PEOPLE` (default 150) people sent to FEC, ~3 calls each at 15/min = ~30 min; ~1,430 resolvable people turn over in ~10 days | Render cron (folded into `ev-jobs-fec-burst`) | `essentials.politicians.finance_summary->>'refreshed_at'` advances on the people it summarised; log line `=== FEC FINANCE SUMMARY RUN COMPLETE ===` with `attempted`/`succeeded`/`deferred`. A run where every attempt failed exits 1. |
