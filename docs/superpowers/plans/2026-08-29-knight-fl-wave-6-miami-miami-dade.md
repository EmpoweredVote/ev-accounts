...
| | |
| --- | --- |
| Worktree | `C:/ev-accounts-knight`, branch `docs/knight-cities-program` |
| Commits | `52c63d09` X0040 · `f6636b21` X0041 · `b764ad33` ROSTERS.md · `475d0aec` generator · `0aaaa4e2` **the wave** |
| Applied to prod | **everything** — the two boundary loads, plus `CC_0015`, `CC_0016`, `CC_0017` |
| Next free migration slots | **`CC_0018`** upward (max on the branch is `CC_0017`) |
| Next free `X` code | `X0042` |
| ⚠ Not yet pushed | `0aaaa4e2` is LOCAL. PR #212 does not show it. |

### ✅ The three migrations are committed under their final numbers

`backend/migrations/CC_0015_miami_structure.sql` (333 lines), `CC_0016_miami_people.sql` (309),
`CC_0017_miami_dade_county.sql` (954), plus `scripts/verify-miami-dade-probes.sql`. The rename was
verified by re-pointing `scripts/gen-miami-dade-migrations.mjs`, regenerating, and diffing all three
**byte-for-byte** — FL-5's rule, and all three reproduced exactly.

### 🔴 What Task 5 measured, for whoever writes the ledger

- **Idempotence proven across all ten** Florida local migrations: every one reaches `COMMIT` with its
  post-verify `NOTICE`, no `ERROR`, every occupancy loop `seated 0`, and the only non-zero `INSERT`s
  are into temp seed tables.
- **`offices_missing_terms` unchanged at 820 / 165 / 655.** The wave adds no vacancy and no
  unflagged row.
- **`check:reachability` adds no new bucket** — 5 `BAD_GEOMETRY` / 17 `DEAD_GEOGRAPHY` /
  38 `UNREACHABLE`, all at baseline.
- **Florida now seats 73 local and county offices, 72 filled, 1 vacant**, across seven governments
  plus the state (164/159/5).
- ⚠ **One probe assertion had to be SCOPED after it read 33 pre-apply.** "Commissioner offices on the
  countywide district" is only zero *for this wave*: Leon and Manatee each seat two at-large
  commissioners countywide and Tallahassee five citywide, all correct. An unscoped ruling counts
  other waves' legitimate rows and reads as a failure.
- ⚠ **The people half cannot be dry-run alone** — its offices do not exist until the structure half
  is applied. It was proven by running structure and people as ONE transaction ending in `ROLLBACK`,
  with the stream asserted to contain zero `COMMIT` first. **Do not simply concatenate the files**;
  that is what committed FL-3's `CC_0008` to prod in autocommit.

### 🔴 Environment facts that cost time this session

1. **`jq` IS NOT INSTALLED.** Use `node -e` for JSON. Several house snippets assume `jq`.
2. **`pdftotext` is Xpdf 4.00, not Poppler** — no `-bbox`. Use **`-table`**; `-layout` misassigns
   every row of the SOE PDF (see the Task 3 deviations).
3. **A `PreToolUse` hook now BLOCKS `export $(...)` and bare `export`/`printenv`/`env`.** It fires
   because `export $(grep … .env)` degrades to a bare `export` — an environment dump — whenever the
   substitution is empty. Use the fail-closed form:
   `cd /c/EV-Accounts/backend && set -a && . ./.env && set +a && psql "$DATABASE_URL" …`
   ⚠ The knight worktree has **no `.env`** (a worktree carries tracked files only); `node_modules`
   IS installed there. Read `DATABASE_URL` from the primary worktree as above.

### 🔴 What Task 5's own gates and probes depend on

- **HD-113 IS VACANT** through November 2026 per the SOE, because Vicki Lopez left it for county
  District 5. **Probe A can never return a state representative at the anchor, and that is correct
  data.** Re-confirm before the apply.
- **FOUR appointed terms, not the two this plan originally predicted** — county D5, D6, D8, D11. The
  emitted gate asserts the count *and* the four titles.
- **ONE person is reused**: Oliver Gilbert, `external_id -1212402`. 19 county terms from **18** new
  politician rows. The county migration asserts the reuse landed as a TERM and that exactly one
  politician row is named Oliver Gilbert.
- **Search the probe file for an apostrophe inside any `\echo` line before running it, and remove
  every one.** FL-5's trap: `\echo` takes the rest of the line raw, but psql still lexes quotes, so
  `Leon's` raised `unterminated quoted string` and swallowed the following `\echo` lines. psql
  **continues past it**, so the probe still ran and still reported PASS — what was lost were the
  headings that say what the numbers mean, which is the whole point of a probe file.

### ⚠ Plan items still open

- **The `n = 93` assertion in Task 3 Step 7 is uninterpretable** and was never implemented. Nothing in
  the wave has 93 rows and no id is 93. **Drop it or define it.**
- **Task 1's loader (`load-miami-dade-commission-boundaries.ts`) still has an unguarded `.json()`.**
  Task 2's has the fix: it reports which layer returned non-JSON and shows the first 200 characters.
  ArcGIS answered a throttled request with an HTML page mid-run.
- **`fl.md`'s FL-1 note that only the congressional map was litigated after 2022** is true of the
  state maps and **false of Miami's city map**. Task 6 corrects it.
...
- [x] **Step 1: Write the two-anchor probe**
...
- [x] **Step 2: Run the probe BEFORE applying**
...
- [x] **Step 3: Dry-run each file separately**
...
- [x] **Step 4: Confirm each rollback reverted**
...
- [x] **Step 5: Take the numbers last, then apply in order**
...
- [x] **Step 6: Re-run ALL TEN Florida local migrations**
