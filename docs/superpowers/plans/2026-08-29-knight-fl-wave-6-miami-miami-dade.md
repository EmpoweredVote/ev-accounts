...
## ▶️ WHERE THIS STOPPED — read this first (2026-08-29)

**Tasks 0–4 are DONE and committed. Task 5 is next: dry-run, rename, apply, gate, probe, commit.**
**Nothing has been applied to prod yet.** `X0040` and `X0041` are LOADED (boundaries only); no
government, chamber, office, politician or term row exists for either jurisdiction.
...
| Commits | `52c63d09` X0040 · `f6636b21` X0041 · `b764ad33` ROSTERS.md · `475d0aec` generator |
| Applied to prod | **only the two boundary loads** — 13 `X0040` + 5 `X0041` rows |
| Next free migration slots | **`CC_0015`, `CC_0016`, `CC_0017`** (max on the branch is `CC_0014`) |
| Next free `X` code | `X0042` |

### 🔴 The three `CC_wip_*.sql` files are ON DISK and UNTRACKED. They are Task 5's input.

`backend/migrations/CC_wip_miami_structure.sql` (333 lines), `CC_wip_miami_people.sql` (309),
`CC_wip_miami_dade_county.sql` (954). Generated by `scripts/gen-miami-dade-migrations.mjs`, which is
committed; the SQL is not, because **the number is taken last** (CLAUDE.md).

⚠ **Do not regenerate blindly.** Rename first, re-point the three `*_FILE` constants at the top of
the generator, then regenerate and diff — FL-5's rule is that a rename is verified by reproducing the
renamed file **byte-for-byte**, not by trusting `sed`.
