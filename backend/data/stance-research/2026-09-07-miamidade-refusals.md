# Miami-Dade — topics read and REFUSED

A refusal is a result. Writing it down stops the next pass paying for the same reading, and
stops a later pass mistaking "no row" for "not looked at". An empty compass is honest; a
confabulated one is a false statement about a real person.

## Marleine Bastien (District 2) — `economic-development` — REFUSED 2026-09-07

**24 leads read. No chair seated. The record does not answer the question this ladder asks.**

The ladder asks how government should **attract businesses**:

| rung | position |
| --- | --- |
| 1 | No tax breaks or subsidies; invest in public services and infrastructure so businesses come on their own |
| 2 | Help small and local businesses grow, but no subsidies to attract large outside companies |
| 3 | Incentives, but only with wage and local-hiring commitments, and clawbacks if undelivered |
| 4 | Large tax breaks and infrastructure for major employers, with limits |
| 5 | The largest incentives for any large employer, with no conditions or spending limits |

Her 24 leads are **Community Redevelopment Agency work**: the N.W. 7th Avenue, N.W. 79th Street,
North Miami, Naranja Lakes and West Perrine CRAs — budgets, findings of necessity, boundary
expansions, board appointments — plus HUD action and consolidated plans, a street improvement, AARP
grant funding and volunteer income tax assistance.

A CRA is a tax-increment district that reinvests incremental property tax inside a designated area.
**That is not a company-specific incentive to attract an employer.** It is a different question, and
the ladder has no rung for it. Direction is not the problem here; the axis is.

Searched all 24 operative titles for `attract`, `major employer`, `job creation`, `targeted jobs`,
`relocat`, `corporate`, `headquarters`, `tax abatement`, `ad valorem tax exemption`, `enterprise
zone`, `opportunity zone`, `business incentive`, `clawback`, `wage requirement`, `local hiring`.
Three matched and **all three are boilerplate**: `CORPORATE` from "A FLORIDA NOT-FOR-PROFIT
CORPORATION", `RELOCAT` from relocating occupants of an acquired parcel.

Two items looked decisive and were not:

- **R-877-25** (`252009`, adopted, her prime sponsorship) — "FEASIBILITY OF IMPLEMENTING
  INCENTIVE-BASED PROGRAMS" is about **waste diversion and recycling**: pay-as-you-throw and waste
  reduction rebates. Matched on the word "incentive". It is a `city-sanitation` item.
- **`250695`** (before the Board, her prime sponsorship) — urges the Florida Legislature to **oppose**
  SB 1242 / HB 991, which would prohibit creating CRAs, bar new CRA projects and debt, and terminate
  existing agencies. This is a real, stated position and it is hers, but it is a position about
  *preserving redevelopment agencies*, not about incentives to attract businesses. It would evidence
  a CRA ladder. There isn't one.

**What would seat this topic later:** an instrument granting or refusing a company-specific
incentive — an ad valorem tax exemption, a targeted-jobs award, a Beacon Council incentive
agreement — or a stated position on conditioning such awards on wages, local hiring or clawbacks.
None exists in the 2025-01-01 → 2026-09-05 window.
...
## `rent-regulation` across the whole Board — 0 seated, 14 refused, and the ladder does not apply here

Fifth topic pass, 2026-09-08. **This one should not be re-attempted for any Florida jurisdiction
without reading this section first.** The refusal is not "we could not find evidence" — it is that
the county is forbidden by state law from doing the thing rungs 1 and 2 describe.

The ladder asks what role government should play in regulating rents and protecting tenants:
...
### 🔴🔴 FLORIDA PREEMPTS LOCAL RENT CONTROL, SO RUNG 5 IS THE STATE'S POSITION, NOT ANYONE'S

**Fla. Stat. § 125.0103(2)**, read at the primary source rather than from memory:

> "A municipality, county, or other entity of local government may not adopt or maintain in effect
> any law, ordinance, rule, or other measure that would have the effect of imposing controls on
> rents."

The housing-emergency exception that used to sit alongside it is **gone from the current text**.
Subsection (3) preserves only land-use measures to increase affordable housing supply, such as
inclusionary housing ordinances — which is the `housing` ladder, not this one.

Three consequences, and the third is the dangerous one:

1. **Rungs 1 and 2 are not things a Miami-Dade commissioner may lawfully do.** No amount of research
   will produce evidence for them, because the instrument would be void.
2. **Rung 3 and rung 4 both describe the preempted baseline**, so neither distinguishes one
   commissioner from another. Every Florida county is already "market rents broadly".
3. 🔴🔴 **RUNG 5 IS THE TRAP. "Oppose rent control entirely; rents should be set by the market
   without government intervention" is an accurate description of the legal status quo in Florida
   and of nobody's stated position.** A reader who works backwards from the outcome will seat all
   thirteen commissioners at 5 and will have recorded a STATE PREEMPTION AS THIRTEEN PERSONAL
   BELIEFS. That is a false statement about thirteen real people, generated by a ladder that is
   valid elsewhere.

This is exactly the per-rung scope ruling in CLAUDE.md — *"a ladder is only valid at a level where
its rungs are things an officeholder there can actually do"* — and `rent-regulation` fails it at
`local` scope **in Florida**. ⚠ It is NOT invalid in general: it carries `local` and `state` roles and
**237 answers exist across the corpus**, presumably in states where rent control is lawful. The
scope defect is jurisdictional, and `compass_topic_roles` has no per-state dimension to express it,
so this note is the control.

**What would legitimately seat a Florida official here:** a *tenant-protection* instrument, which is
not preempted — a tenants' bill of rights, a notice-of-rent-increase requirement, an
eviction-diversion or right-to-counsel programme, a source-of-income anti-discrimination ordinance.
Rung 3's "maintain current tenant protections" is reachable that way. None was adopted in the
2025-01-01 → 2026-09-05 window.

### 🔴 THE PATTERN WAS 23 OF 23 FALSE POSITIVES, AND ONE WORD CAUSED ALL OF THEM

Every matter this ladder produced was a **county property lease**: airport development leases with
KASE, AIR SAL, CR Aviation, MQ TMB Holdings, Bunker Aviation and Carisma Air; an office lease for the
Sheriff; the Department of Health; the Haulover restaurant; a fire station for FDOT; a legacy donor
lease; the Seaquarium; and a room for Suited for Success, Inc.

**A county is a landlord constantly.** Every one of those documents says "MIAMI-DADE COUNTY, AS
LANDLORD", and not one is a position on regulating rents.

🟢 **MEASURED PER-ALTERNATIVE RATHER THAN GUESSED**, which is what made the fix surgical: each of the
seven alternatives was run separately over the unfiltered sweep.

| alternative | leads |
| --- | --- |
| `landlord` | **30** |
| `rent control`, `rent stabiliz`, `rent increase`, `tenant…right\|protection`, `eviction`, `just cause` | **0 each** |

**23 of 23 matters had `landlord` as their only reason to match.** So the lease sense is removed, the
regulatory sense kept as `landlord-tenant`, and `tenants bill of rights` added because that is what a
Florida county can actually adopt.

Verified in both directions before landing: **7 real instruments still match** (rent stabilization,
repealing rent control, a tenants' bill of rights, a rent-increase notice, just-cause eviction, a
landlord-tenant code amendment, tenant protections in county-assisted housing) and **3 county leases
no longer do**. Miami-Dade goes from 30 leads to **0**, which is the true answer. Pinned by two tests.

⚠ **A zero is only readable because the scan carries a positive control** — 2,543 hits on
"resolution"/"ordinance" in the same run. `miamidade-axis-control.mjs` refuses to report a zero
without it, and that is why this section can claim absence rather than silence.

### The Board, for the record

Nothing to attribute. No commissioner sponsored a rent-control, rent-stabilization, tenants'-bill-of-
