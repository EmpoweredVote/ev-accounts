...
change is target-selection: TN's new-candidate external_id band -470910..-470101 from 161-06.
...
Usage:
  python backend/scripts/seed-tn-house-headshots.py --state TN [--dry-run]
  python backend/scripts/seed-tn-house-headshots.py --manual <file>   (external_id|image_url|license)
Writes backend/scripts/_tn-house-headshot-results.json.
"""
...
# TN band: -470910..-470101 (161-06 generator output; excludes incumbents -47001..-47009 which fall
# far outside this band). Election name is NOT used for scoping (see module docstring, D-01b).
BANDS = {
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, tennessee|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_tn-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state TN required (or --manual <file>)'); sys.exit(1)
    lo, hi, sname = BANDS[state]

    conn = psycopg2.connect(DATABASE_URL); conn.autocommit = False
    cur = conn.cursor()
    # New TN 2026 House candidates: scoped by external_id BAND ONLY (no election-name join). Unlike
    # OH/GA/NC/MI (single election per state), TN has TWO elections (general + withheld "Polygon
    # Pending" for the 5 severe/redistricted districts per 161-06 D-01b) -- severe-district candidates
    # STILL need headshots even though their race doesn't currently surface on /elections. The band
    # filter still excludes reused incumbents (their external_ids like -47001 fall OUTSIDE -470101..-470910).
    cur.execute("""
