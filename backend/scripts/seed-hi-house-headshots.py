...
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", hawaii" place-token in
...
BANDS = {
    # HI new-candidate band (165-06: -150101..-150206). HI incumbents (-15001/-15002) outside band.
    'HI': (-150299, -150101, 'Hawaii'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, hawaii|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_hi-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state HI required (or --manual <file>)'); sys.exit(1)
