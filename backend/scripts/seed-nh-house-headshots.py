...
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", new hampshire" place-token in
...
BANDS = {
    # NH new-candidate band (165-06: -330133..-330146 NH-1, -330201..-330205 NH-2). Upper bound -330133 EXCLUDES the 5 NH local officials at -330128..-330132 (parallel-session domain).
    'NH': (-330299, -330133, 'New Hampshire'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, new hampshire|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_nh-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state NH required (or --manual <file>)'); sys.exit(1)
