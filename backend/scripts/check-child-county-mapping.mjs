 ...
 *
 * NOTHING ENFORCES THAT. 52 scripts insert into essentials.geofence_boundaries, so there is no
 * single loader to hook, and a trigger on a bulk geometry load is its own problem. The read path
 * already degrades safely — an unmapped child gets a NULL county rather than being dropped — and
 * warns in the API log. But a warning in a log nobody tails is not a guard, and the symptom is
 * quiet: a newly loaded city silently sits outside its county on the coverage dashboard.
 *
 ...
 *
 * Needs a live DB, so like check:reachability this does NOT run on PRs — it runs on master pushes
