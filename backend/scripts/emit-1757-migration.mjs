// The last four ORPHAN_CONTEXT rows above baseline: Berkeley council rows blanked by 1738/1739.
// Captures the current reasoning verbatim, then emits 1757 rewriting each as a documented blank whose
// text is the finding the blanking migration already recorded.
//
// Same remedy as 1756 and for the same reason: the topics genuinely apply to a Berkeley council member,
// and the looking is on the record -- 1738 read all 52 of Blackaby's items, and 1739 established that
// Tregub's absence rests on a COMPLETE record (seated December 2024; none of the 96 items recovered by
// the corpus repair falls inside that tenure). Without that last fact the blank would be a gap wearing
// a finding's clothes.
...
const TOPIC_PUBLIC_SAFETY = 'e9ebefcd-c496-45e8-b816-a79f8442ba85';

// Findings transcribed from the headers of 1738 and 1739. Possessives are dropped rather than carried:
// these are voter-facing rows and nothing in the corpus states these people's pronouns.
const ROWS = [
  {
    pid: '424eb63b-9976-4059-8049-365c09719cc6',
    tid: '44905f3b-e105-4f6c-afc7-5d223813dbac',
    who: 'Brent Blackaby / Deportation',
    mig: '1738',
    text:
      'Researched 2026-08-13 — all 52 council items read, and the row fails for two independent ' +
      'reasons. The ladder asks WHO should be deported: chair 2 is "only deport people convicted of ' +
      'serious violent crimes", but the ordinance backed here is CATEGORICAL, carrying no ' +
      'criminal-conviction carve-out anywhere, which contradicts that chair rather than supporting ' +
      'it. And the cited instrument, the $200,000 deportation defence fund, is the same act already ' +
      'carrying the Local Immigration Enforcement row — a city instrument answers whether the city ' +
      'cooperates, not who should be deported. No scorable public record was found for this chair.',
  },
  {
    pid: '9f9a35a9-0226-45f0-9fd8-ef46163f7245',
    tid: TOPIC_PUBLIC_SAFETY,
    who: 'Igor Tregub / Public Safety Approach',
    mig: '1738',
    text:
      'Researched 2026-08-13 — chair 3 is "adding crisis response teams". The cited Proposition 6 ' +
      'resolution is about FORCED PRISON LABOUR, and the only Specialized Care Unit item in this ' +
      'tenure is a City Manager contract to EVALUATE the existing unit (2024-11-19). No scorable ' +
      'public record was found for this chair.',
  },
  {
    pid: 'bcdb549a-48bf-400f-9d23-c93e2e71007c',
    tid: TOPIC_PUBLIC_SAFETY,
    who: 'Terry Taplin / Public Safety Approach',
    mig: '1738',
    text:
      'Researched 2026-08-13 — chair 2 is "shift non-violent calls to unarmed mental health ' +
      'co-responders". The sole mental-health item in the record is a CEREMONIAL PROCLAMATION, ' +
      'Res. 69,853-N.S. declaring May 2021 Mental Health Month: on-topic by vocabulary, not by ' +
      'rationale. No scorable public record was found for this chair.',
  },
  {
    pid: '9f9a35a9-0226-45f0-9fd8-ef46163f7245',
    tid: 'ba59337e-30e2-4aba-a39a-426b3366eb27',
    who: 'Igor Tregub / Transportation Priorities',
    mig: '1739',
    text:
      'Researched 2026-08-13 — chair 2\'s "invest equally in roads" is CONTRADICTED by a record that ' +
      'is pedestrian, cycling and transit throughout (the Oxford for All Class IV bikeway, accessible ' +
      'pedestrian signals, the MTC letter). Chair 1 would fit that half, but its second clause, ' +
      '"reduce parking requirements communitywide", has no instrument behind it anywhere in the ' +
      'repaired corpus. Contradicted at 2, incomplete at 1, so neither chair is described. The ' +
      'absence rests on a COMPLETE record: seated December 2024, and none of the 96 items recovered ' +
      'by the corpus repair falls inside that tenure. No scorable public record was found for this ' +
      'chair.',
  },
];

const CARVE_DATE = /^researched\s+[0-9]{4}-[0-9]{2}-[0-9]{2}/i;
const CARVE_PROSE = /no scorable/i;
for (const r of ROWS) {
  if (!CARVE_DATE.test(r.text) || !CARVE_PROSE.test(r.text)) {
    throw new Error(`composed text would still be counted by the gate: ${r.who}`);
  }
}

const { rows: current } = await pool.query(
  `SELECT politician_id, topic_id, reasoning, sources FROM inform.politician_context
    WHERE (politician_id, topic_id) IN (${ROWS.map((r) => `('${r.pid}','${r.tid}')`).join(',')})`,
);
await pool.end();

if (current.length !== ROWS.length) {
  throw new Error(`expected ${ROWS.length} rows in politician_context, found ${current.length}`);
}

const values = ROWS.map((r) => `  ('${r.pid}','${r.tid}',$bk$${r.text}$bk$)`).join(',\n');

const sql = `-- 1757_ca_berkeley_orphan_context_blanks.sql
...
BEGIN
  ...
  IF n <> ${ROWS.length} THEN RAISE EXCEPTION 'pre-check: % rows, expected ${ROWS.length}', n; END IF;
...
writeFileSync('migrations/1757_ca_berkeley_orphan_context_blanks.sql', sql);

const byKey = new Map(current.map((c) => [`${c.politician_id}|${c.topic_id}`, c]));
writeFileSync(
  'data/stance-retirement/2026-08-14-ca-berkeley-orphan-1757-rollback.json',
  JSON.stringify(
    {
      pass: 'Berkeley orphan context — reasoning rewritten as documented blanks',
      migration: '1757_ca_berkeley_orphan_context_blanks.sql',
      note:
        'reasoning REPLACED, sources untouched, no answer created or deleted. Each new text restates ' +
        'the finding recorded in the header of the migration that blanked the answer (1738 or 1739). ' +
        'Restoring is an UPDATE from prior_reasoning.',
      n_rows: ROWS.length,
      rows: ROWS.map((r) => ({
        politician_id: r.pid,
        topic_id: r.tid,
        who: r.who,
        blanked_by: r.mig,
        prior_reasoning: byKey.get(`${r.pid}|${r.tid}`)?.reasoning ?? null,
        new_reasoning: r.text,
        sources: byKey.get(`${r.pid}|${r.tid}`)?.sources ?? null,
      })),
    },
    null,
    1,
...
console.log(`emitted 1757 with ${ROWS.length} rewrites`);
for (const r of ROWS) console.log(`\n${r.who}  (blanked by ${r.mig})\n  ${r.text}`);
