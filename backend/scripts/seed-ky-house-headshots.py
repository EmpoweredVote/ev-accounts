"""
seed-ky-house-headshots.py — Phase 164-04 (v2.22 Wave 3). CLONE of seed-ks-house-headshots.py, all
guards intact incl. Phase-156 hardening (dropped 'american' kw + pre-1940 historical-year reject).
Only changes: KY band, KY state name, RESULTS_JSON, and the localized ", kentucky" place-token in
...
BANDS = {
    # KY new-candidate band (164-04 generator: actual range -210604..-210201, incl KY-1 seq-200
    # at -210300). Upper bound is -210201 (KY-2 seq 1) to EXCLUDE the 98 MA state legislators that
    # occupy KY-1 seqs 1-98 (-210101..-210198, D-04 collision) — we never touch those. KY
    # incumbents (-21001..-21006) are far outside this band. Guards unchanged.
    'KY': (-210899, -210201, 'Kentucky'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, kentucky|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_ky-house-headshot-results.json')
