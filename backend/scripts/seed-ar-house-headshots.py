"""
seed-ar-house-headshots.py — Phase 164-06 (v2.22 Wave 3). CLONE of seed-ks-house-headshots.py, all
guards intact incl. Phase-156 hardening (dropped 'american' kw + pre-1940 historical-year reject).
Only changes: AR band, AR state name, RESULTS_JSON, and the localized ", arkansas" place-token in
...
BANDS = {
    # AR new-candidate band (164-06 generator: actual range -50401..-50101). Band confirmed EMPTY
    # live before seeding (164-06 Task 1). AR incumbents (-5001..-5004) are 4-digit ids outside
    # this 5-digit band (already imaged). Guards unchanged.
    'AR': (-50499, -50101, 'Arkansas'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, arkansas|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_ar-house-headshot-results.json')
