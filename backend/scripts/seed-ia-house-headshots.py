"""
seed-ia-house-headshots.py — Phase 164-05 (v2.22 Wave 3). CLONE of seed-ks-house-headshots.py, all
guards intact incl. Phase-156 hardening (dropped 'american' kw + pre-1940 historical-year reject).
Only changes: IA band, IA state name, RESULTS_JSON, and the localized ", iowa" place-token in
...
BANDS = {
    # IA new-candidate band (164-05 generator: actual range -190402..-190101). The band was
    # confirmed EMPTY live before seeding (164-05 Task 1) — no legacy collisions. IA incumbents
    # (-19001..-19004) fall far outside this band. Guards unchanged.
    'IA': (-190499, -190101, 'Iowa'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, iowa|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_ia-house-headshot-results.json')
