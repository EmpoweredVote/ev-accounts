"""
seed-nj-house-headshots.py — Phase 157 (v2.21 Wave 2) plan 157-04. CLONE of seed-oh-ga-nc-house-headshots.py (156-06), all guards intact incl. 156 hardening (dropped american kw + pre-1940 historical-year reject). Only change: NJ band added.
...
BANDS = {
    'OH': (-399999, -390000, 'Ohio', 'OH 2026 Statewide General'),
    'GA': (-139999, -130000, 'Georgia', 'GA 2026 Statewide General'),
    'NC': (-379999, -370000, 'North Carolina', 'NC 2026 Statewide General'),
    'NJ': (-341299, -340101, 'New Jersey', 'NJ 2026 Statewide General'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, texas|, new york|, ohio|, georgia|, north carolina|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_nj-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state NJ required (or --manual <file>)'); sys.exit(1)
    lo, hi, sname, election = BANDS[state]

    conn = psycopg2.connect(DATABASE_URL); conn.autocommit = False
    cur = conn.cursor()
    # New OH/GA/NC 2026 House candidates: scoped to the state's election via race_candidates (the raw
    # negative band is POLLUTED with unrelated records — e.g. CA-gubernatorial homonyms — so a bare band
    # query would touch non-2026-House rows). The band filter still excludes reused incumbents (their
    # external_ids like -39001 fall OUTSIDE -39xxxx). Phase-156 fix.
    cur.execute("""
        ...
        WHERE el.name = %s
          AND p.external_id BETWEEN %s AND %s
