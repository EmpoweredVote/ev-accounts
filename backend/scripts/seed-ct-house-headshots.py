"""
seed-ct-house-headshots.py — Phase 164-02 (v2.22 Wave 3). CLONE of seed-ks-house-headshots.py, all
guards intact incl. Phase-156 hardening (dropped 'american' kw + pre-1940 historical-year reject).
Only changes: CT band, CT state name, RESULTS_JSON, and the localized ", connecticut" place-token in
...
BANDS = {
    # CT new-candidate band (164-02 generator: actual range -90504..-90101). No collisions in this
    # band (164-02 Task 1 live check). CT incumbents (-9001..-9005) are 4-digit ids far outside this
    # 5-digit band (already imaged). Same hardened guards.
    'CT': (-90599, -90101, 'Connecticut'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, connecticut|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_ct-house-headshot-results.json')
