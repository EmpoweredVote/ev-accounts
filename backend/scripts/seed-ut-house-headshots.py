"""
seed-ms-house-headshots.py — Phase 164-06 (v2.22 Wave 3). CLONE of seed-ks-house-headshots.py, all
guards intact incl. Phase-156 hardening (dropped 'american' kw + pre-1940 historical-year reject).
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", utah" place-token in
...
BANDS = {
    # UT new-challenger band (165-02 generator: actual range -490402..-490101). Band confirmed EMPTY
    # live before seeding (165-02 Task 1). UT incumbents + reused primary pids have NO external_id and
    # thus fall outside the band naturally (already imaged). Guards unchanged.
    'UT': (-490499, -490101, 'Utah'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, utah|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_ut-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state UT required (or --manual <file>)'); sys.exit(1)
