...
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", wyoming" place-token in
...
BANDS = {
    # WY new-candidate band (165-07: -560001..-560013). Chuck Gray (v2.18 pid, no external band id) + Hageman outside band.
    'WY': (-560099, -560001, 'Wyoming'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, wyoming|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_wy-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state WY required (or --manual <file>)'); sys.exit(1)
