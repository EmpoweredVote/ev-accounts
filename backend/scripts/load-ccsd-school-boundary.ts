/**
 * load-ccsd-school-boundary.ts
 *
 * Downloads the Nevada TIGER UNSD shapefile from census.gov, filters to the
 * single Clark County School District (GEOID 3200060), and inserts a
 * G5420 geofence_boundaries row into essentials.geofence_boundaries.
 *
 * The district is stored with:
 *   geo_id  = GEOID value directly ('3200060') — NOT a slug
 *   mtfcc   = 'G5420'
 *   state   = '32' (Nevada FIPS)
 *   source  = 'tiger_unsd_nv_2024'
 *
 * CCSD is a county-wide unified district, so the polygon ~= Clark County.
 * Safe to re-run — ON CONFLICT (geo_id, mtfcc) DO NOTHING ensures idempotency.
 *
 * Usage (from C:/EV-Accounts/backend):
 *   npx tsx scripts/load-ccsd-school-boundary.ts --dry-run
 *   npx tsx scripts/load-ccsd-school-boundary.ts
 */
...
const TIGER_URL    = 'https://www2.census.gov/geo/tiger/TIGER2024/UNSD/tl_2024_32_unsd.zip';
const MTFCC        = 'G5420';
const STATE        = '32';           // Nevada FIPS — geofence_boundaries.state convention
const SOURCE       = 'tiger_unsd_nv_2024';
const EXPECTED_COUNT = 1;

// geo_id = GEOID field value directly ('3200060') — NCES 3200060 / Census sch3200060.
...
const baseName = 'tl_2024_32_unsd';
const tmpRoot  = path.join(process.cwd(), '.tmp-nv-school-unsd');
...
async function main() {
  console.log('[load-ccsd-school-boundary] Fetching NV UNSD boundary (Clark County School District)');
  ...
  try {
    ...
    // Step 5: Assert the target GEOID was found
    ...
    if (!DRY_RUN) {
      ...
      if (total !== EXPECTED_COUNT) {
        ...
        console.log(`  All ${EXPECTED_COUNT} NV school district boundaries loaded successfully.`);
      }
    }
  ...
  console.log('\n[load-ccsd-school-boundary] Done.');
}

main()
  .catch((err) => {
    console.error('[load-ccsd-school-boundary] Fatal error:', err);
    process.exit(1);
  })
