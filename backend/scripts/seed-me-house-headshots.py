...
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", maine" place-token in
...
BANDS = {
    # ME new-candidate band (165-01: Russell -230103, Dunlap -230203). Already-imaged legacy in-band
    # records (Collins/King/Pingree/Golden) are skipped by the NOT EXISTS politician_images guard.
    'ME': (-230299, -230101, 'Maine'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, maine|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_me-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state ME required (or --manual <file>)'); sys.exit(1)
