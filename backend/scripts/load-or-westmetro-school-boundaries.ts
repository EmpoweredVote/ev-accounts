 ...
 *
 * Downloads the Oregon TIGER UNSD shapefile from census.gov, filters to the
 * 5 target west-metro Washington County school districts by GEOID, and inserts
 * G5420 geofence_boundaries rows into essentials.geofence_boundaries.
 *
 * Each district is stored with:
 *   geo_id  = GEOID value directly (e.g. '4101920') — NOT a slug like LAUSD
 *   mtfcc   = 'G5420'
 *   state   = '41' (Oregon FIPS)
 *   source  = 'tiger_unsd_or_2024_westmetro'
 *
...
const MTFCC        = 'G5420';
const STATE        = '41';           // Oregon FIPS — geofence_boundaries.state convention
const SOURCE       = 'tiger_unsd_or_2024_westmetro';
const EXPECTED_COUNT = 5;

// geo_id = GEOID field value directly (e.g. '4101920') — NOT a slug like LAUSD's 'lausd-board-district-N'
// All 5 GEOIDs verified via TIGERweb REST API (layer 10 — Unified School Districts) 2026-06-30.
...
async function main() {
  console.log('[load-or-westmetro-school-boundaries] Fetching OR west-metro UNSD boundaries');
  ...
  try {
    ...
    while (!result.done) {
      ...
      // Log field names on first feature for diagnostic purposes
      if (firstFeature) {
        firstFeature = false;
  ...
  console.log('\n[load-or-westmetro-school-boundaries] Done.');
}
