...
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", nebraska" place-token in
...
BANDS = {
    # NE new-candidate band (165-04: -310101..-310361). NE incumbents (-31001/-31003) outside band.
    'NE': (-310399, -310101, 'Nebraska'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, nebraska|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_ne-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state NE required (or --manual <file>)'); sys.exit(1)
