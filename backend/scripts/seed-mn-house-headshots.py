"""
seed-mn-house-headshots.py — Phase 162-05 (v2.22). CLONE of seed-ma-house-headshots.py
(157-04 lineage), all guards intact incl. 156 hardening (dropped american kw + pre-1940
historical-year reject). Only change: MN band added, covering ONLY the new -270xxx MN
challenger/open-seat/independent external_id band (-270899..-270101). Incumbents
(-27001..-27008) are OUTSIDE this band and excluded (Craig MN-2 has no active row at all).
...
Usage:
  python backend/scripts/seed-mn-house-headshots.py --state MN [--dry-run]
  python backend/scripts/seed-mn-house-headshots.py --manual <file>   (external_id|image_url|license)
Writes backend/scripts/_mn-house-headshot-results.json.
"""
...
# MN band covers ONLY the 35 new 162-05 challenger/open-seat/independent records (-270899..-270101);
# renominated incumbents (-27001..-27008) are outside this band and excluded.
BANDS = {
    'MN': (-270899, -270101, 'Minnesota', 'MN 2026 Statewide General'),
}
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_mn-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state MN required (or --manual <file>)'); sys.exit(1)
