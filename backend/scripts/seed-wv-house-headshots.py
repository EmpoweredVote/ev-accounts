...
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", west virginia" place-token in
...
BANDS = {
    # WV new-challenger band (165-05: -540101..-540203). WV incumbents (-54001/-54002) outside band.
    'WV': (-540299, -540101, 'West Virginia'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, west virginia|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_wv-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state WV required (or --manual <file>)'); sys.exit(1)
