...
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", north dakota" place-token in
...
BANDS = {
    # ND new-candidate band (165-08: -380001 Hammer). Fedorchak (-38000) outside band.
    'ND': (-380099, -380001, 'North Dakota'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, north dakota|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_nd-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state ND required (or --manual <file>)'); sys.exit(1)
