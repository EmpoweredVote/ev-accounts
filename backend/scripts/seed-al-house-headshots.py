"""
seed-wi-house-headshots.py — Phase 163-02 (v2.22 Wave 3). CLONE of seed-mo-house-headshots.py, all
guards intact incl. Phase-156 hardening (dropped 'american' kw + pre-1940 historical-year reject).
Only changes: WI band, WI state name, RESULTS_JSON, and the localized ", alabama" place-token in
...
target-selection change is WI's new-candidate external_id band -550899..-550101 from 163-02.
New challengers have NO essentials.offices row, so the band query is used (not an offices-join).

NOTE (163-02): WI is NOT redistricted -- no withholding; all 8 WI races surface on the plain
'WI 2026 Statewide General' election. Incumbents (-55001..-55008) are excluded by construction
...
Usage:
  python backend/scripts/seed-wi-house-headshots.py --state WI [--dry-run]
  python backend/scripts/seed-wi-house-headshots.py --manual <file>   (external_id|image_url|license)
Writes backend/scripts/_al-house-headshot-results.json.
"""
...
# AL band: -10799..-10101 (163-04 generator output, actual range -10702..-10101; band-scoped only -- NO election-name join so severe-district (withheld AL-2) candidates still get headshots; excludes
# incumbents -1001..-1007 which fall far outside this band). Election name is NOT used for scoping.
BANDS = {
    'AL': (-10799, -10101, 'Alabama'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, alabama|'
...
# 163-02 Task 3 hardening (Rule 1 - bug fix): "Douglas Alexander" resolved to a British Labour MP
# (Wikipedia short description "British politician (born 1967)") -- the word "politician" satisfied
# POLITICAL_KW with no nationality check, uploading a wrong-person headshot for the WI-2 challenger.
# Reject any description naming a non-US nationality unless it also names the target US state
# (mirrors the existing _desc_is_historical pattern: additive negative-disambiguation check).
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_al-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state WI required (or --manual <file>)'); sys.exit(1)
    lo, hi, sname = BANDS[state]

    conn = psycopg2.connect(DATABASE_URL); conn.autocommit = False
    cur = conn.cursor()
    # New WI 2026 House candidates: scoped by external_id BAND ONLY (no election-name join). WI has
    # a single election (vanilla, no withholding) -- the band filter still excludes reused incumbents
    # (their external_ids like -55001 fall OUTSIDE -550101..-550899).
    cur.execute("""
