...
Only changes: WI band, WI state name, RESULTS_JSON, and the localized ", louisiana" place-token in
...
Writes backend/scripts/_la-house-headshot-results.json.
"""
...
# LA band: -220699..-220101 (163-06 generator output, actual range -220604..-220101; band-scoped only, NO election join so withheld LA-2 + LA-6 candidates still get headshots; excludes
# incumbents -22001..-22006 which fall far outside this band). Election name is NOT used for scoping.
BANDS = {
    'LA': (-220699, -220101, 'Louisiana'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, louisiana|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_la-house-headshot-results.json')
