 ...
 *
 * Downloads the Texas TIGER UNSD shapefile from census.gov, filters to the
 * 5 target Collin County school districts by GEOID, and inserts
 * G5420 geofence_boundaries rows into essentials.geofence_boundaries.
 *
 * Each district is stored with:
 *   geo_id  = GEOID value directly (e.g. '4835100') — NOT a slug like LAUSD
 *   mtfcc   = 'G5420'
 *   state   = '48' (Texas FIPS)
...
// geo_id = GEOID field value directly (e.g. '4835100') — NOT a slug like LAUSD's 'lausd-board-district-N'
// All 5 GEOIDs verified via TIGERweb REST API (layer 10) 2026-06-02.
...
async function main() {
  console.log('[load-tx-school-boundaries] Fetching TX UNSD boundaries');
  ...
  try {
    ...
    if (!DRY_RUN) {
      ...
      if (total !== EXPECTED_COUNT) {
        ...
        console.log(`  All ${EXPECTED_COUNT} TX school district boundaries loaded successfully.`);
      }
    }
  ...
  console.log('\n[load-tx-school-boundaries] Done.');
}
