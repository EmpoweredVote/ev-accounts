...
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", vermont" place-token in
...
BANDS = {
    # VT band LOWER-BOUNDED at -500006 (safe_start_seq=6): seqs 1-5 = unrelated records, untouched.
    'VT': (-500099, -500006, 'Vermont'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, vermont|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_vt-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state VT required (or --manual <file>)'); sys.exit(1)
