...
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", montana" place-token in
...
BANDS = {
    # MT new-candidate band (165-08: -300101..-300103, -300285/-300286). Seq 84 legacy record + incumbents outside effective targets (no-image guard).
    'MT': (-300299, -300101, 'Montana'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, montana|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_mt-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state MT required (or --manual <file>)'); sys.exit(1)
