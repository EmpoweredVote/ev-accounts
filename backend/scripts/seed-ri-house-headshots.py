...
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", rhode island" place-token in
...
BANDS = {
    # RI new-challenger band (165-07: -440101..-440202). RI incumbents (-44001/-44002) outside band.
    'RI': (-440299, -440101, 'Rhode Island'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, rhode island|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_ri-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state RI required (or --manual <file>)'); sys.exit(1)
