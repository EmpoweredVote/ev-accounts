...
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", new mexico" place-token in
...
BANDS = {
    # NM new-challenger band (165-04: -350126/-350251/-350382). NM incumbents (-35001..-35003) outside band.
    'NM': (-350399, -350101, 'New Mexico'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, new mexico|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_nm-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state NM required (or --manual <file>)'); sys.exit(1)
