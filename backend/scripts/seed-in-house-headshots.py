"""
seed-in-house-headshots.py — Phase 162-07 (v2.22). CLONE of seed-ma-house-headshots.py
(157-04 lineage), all guards intact incl. 156 hardening (dropped american kw + pre-1940
historical-year reject). Only change: IN band added, covering the -180xxx IN challenger
external_id band (-180902..-180101) — this includes both the 5 genuinely-new records AND the
7 reused discovery/bulk-seed records (assigned band external_ids in migration 1214). IN
incumbents use mixed -18nnn + positive SoS ids (499386/499408/499413/499417), all OUTSIDE the
negative -180xxx band and excluded by construction.
...
Usage:
  python backend/scripts/seed-in-house-headshots.py --state IN [--dry-run]
  python backend/scripts/seed-in-house-headshots.py --manual <file>   (external_id|image_url|license)
Writes backend/scripts/_in-house-headshot-results.json.
"""
...
# IN band covers the 12 162-07 challengers (5 new + 7 reused, all assigned -180xxx in mig 1214);
# renominated incumbents (mixed -18nnn + positive SoS ids) are outside this negative band, excluded.
BANDS = {
    'IN': (-180902, -180101, 'Indiana', 'IN 2026 Statewide General'),
}
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_in-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state IN required (or --manual <file>)'); sys.exit(1)
    lo, hi, sname, election = BANDS[state]

    conn = psycopg2.connect(DATABASE_URL); conn.autocommit = False
    cur = conn.cursor()
    # New MA 2026 House candidates: scoped to the state's election via race_candidates (the raw
    # negative band can be POLLUTED with unrelated records elsewhere in the DB, so a bare band
    # query would be unsafe) -- band filter still excludes reused incumbents (-200201..-200209
    # fall OUTSIDE -250902..-250101), and Clark/Pressley (UUID pids only) are never in scope.
    cur.execute("""
