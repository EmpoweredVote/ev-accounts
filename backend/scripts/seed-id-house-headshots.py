...
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", idaho" place-token in
...
BANDS = {
    # ID new-challenger band (165-05: -160101..-160205). ID incumbents (-16001/-16002) outside band.
    'ID': (-160299, -160101, 'Idaho'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, idaho|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_id-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state ID required (or --manual <file>)'); sys.exit(1)
