...
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", mississippi" place-token in
...
BANDS = {
    # MS new-candidate band (164-06 generator: actual range -280402..-280101). Band confirmed EMPTY
    # live before seeding (164-06 Task 1). MS incumbents (-28001..-28004) are 5-digit ids outside
    # this band (already imaged). Guards unchanged.
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, mississippi|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_ms-house-headshot-results.json')
