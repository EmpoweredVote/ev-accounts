 ...
 *
 * Downloads the Indiana TIGER UNSD shapefile from census.gov, filters to the
 * 2 target school districts by GEOID, and inserts
 * G5420 geofence_boundaries rows into essentials.geofence_boundaries.
 *
 * Each district is stored with:
 *   geo_id  = GEOID value directly (e.g. '1804770') — NOT a slug like LAUSD
 *   mtfcc   = 'G5420'
...
const MTFCC        = 'G5420';
const STATE        = '18';           // Indiana FIPS — geofence_boundaries.state convention
const SOURCE       = 'tiger_unsd_in_2024';
const EXPECTED_COUNT = 2;

// geo_id = GEOID field value directly (e.g. '1804770') — NOT a slug like LAUSD's 'lausd-board-district-N'
// GEOIDs verified via NCES CCD + data.census.gov 2026-06-03.
// CRITICAL: 1804770 = Indianapolis Public Schools (Marion County)
// CRITICAL: 1800630 = Monroe County Community School Corporation (Bloomington)
// DO NOT swap — wrong mapping routes Indianapolis residents to Bloomington's board.
...
async function main() {
  console.log('[load-in-school-boundaries] Fetching IN UNSD boundaries');
  ...
  try {
    ...
    // Step 5: Assert all 2 GEOIDs were found
    ...
    if (!DRY_RUN) {
      ...
      if (total !== EXPECTED_COUNT) {
        ...
        console.log(`  All ${EXPECTED_COUNT} IN school district boundaries loaded successfully.`);
      }
    }
  ...
  console.log('\n[load-in-school-boundaries] Done.');
}

main()
  .catch((err) => {
    console.error('[load-in-school-boundaries] Fatal error:', err);
    process.exit(1);
  })
