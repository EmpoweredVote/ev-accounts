 ...
 * all 16 people before a line of SQL is written.
 *
...
const CTAG = 'muscogee county';
const CSEED = 'msc_seed';
...
--
-- ---------------------------------------------------------------------------
-- 🔴 ${undated.length} TERMS ARE OPEN-ENDED AT 'unknown'; ${dated.length} IS DATED AT 'month'.
--
...
-- ⚠ SCOPED TO THIS MIGRATION'S OWN SUB-RANGE ${bandMin}..${bandMax}, NOT to the wave
-- band ${r.id_band.min}..${r.id_band.max}. Task 3 owns ${r.city.offices.map((o) => o.person.external_id).sort((a, b) => a - b)[0]}..${r.city.offices.map((o) => o.person.external_id).sort((a, b) => a - b).slice(-1)[0]}; a guard over the whole
...
BEGIN
  ...
  -- 🔴 EXACTLY ${dated.length} ROW IS DATED, AND IT IS THE TAX COMMISSIONER.
  ...
  SELECT count(*) INTO v_n FROM ${CSEED}
   WHERE term_start IS NOT NULL AND ext_id NOT IN (${dated.map((o) => o.person.external_id).join(', ')});
  IF v_n <> 0 THEN RAISE EXCEPTION '${CTAG} payload: % dated row(s) are not the expected one', v_n; END IF;
...
-- REFUSES a NULL term_start, and ${undated.length} of these ${offices.length} have no published start of any
...
BEGIN
  ...
  LOOP
    ...
    END IF;
  END LOOP;
  RAISE NOTICE '${CTAG}: seated % dated officer(s) via the helper, % with an honest unknown start', v_seated, v_blank;
END $$;
...
${datedChecks}
  ...
  RAISE NOTICE '${CTAG} OK: 1 chamber, ${offices.length} offices, ${offices.length} officers seated, 0 vacancies, 0 districts created';
END $$;
...
const nDatedCity = city.offices.filter((o) => o.person.term_start).length;
const nDatedCty = county.offices.filter((o) => o.person.term_start).length;
console.log(
  `  structure: ${districtRows().length} districts, 1 government, ${city.chambers.length} chambers, ${city.offices.length} offices`,
);
console.log(
  `  people:    ${city.offices.length} politicians, ${city.offices.length} terms (${nDatedCity} dated, ${city.offices.length - nDatedCity} open-ended 'unknown')`,
);
console.log(
  `  county:    1 chamber, ${county.offices.length} offices, ${county.offices.length} politicians, ${county.offices.length} terms (${nDatedCty} dated, ${county.offices.length - nDatedCty} open-ended 'unknown'), 0 districts created`,
);
console.log('\n🔴 The migration NUMBERS are taken LAST — re-count against every remote ref.');
console.log('🔴 Dry-run all three as ONE transaction; assert on ^\\s*COMMIT\\s*; not a substring count.');
