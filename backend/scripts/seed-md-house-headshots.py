"""
seed-md-house-headshots.py — Phase 162-08 (v2.22). CLONE of seed-ma-house-headshots.py
(157-04 lineage), all guards intact incl. 156 hardening (dropped american kw + pre-1940
historical-year reject). Only change: MA band added, covering ONLY the new -25xxxx MA
challenger/open-seat/declared-independent external_id band (-250902..-250101). Incumbents
(-200201..-200209) are OUTSIDE this band and excluded. Clark (MA-5) and Pressley (MA-7) already
have politician_images rows from prior phases and are not touched (their pids are UUID-keyed,
not in the -25xxxx negative external_id band at all).
...
Usage:
  python backend/scripts/seed-md-house-headshots.py --state MD [--dry-run]
  python backend/scripts/seed-md-house-headshots.py --manual <file>   (external_id|image_url|license)
Writes backend/scripts/_md-house-headshot-results.json.
"""
...
# MA band covers ONLY the 18 new 161-08 challenger/open-seat/independent records (-250902..-250101);
# renominated incumbents (-200201..-200209) are outside this band and excluded.
BANDS = {
    'MD': (-240899, -240101, 'Maryland', '2026 Maryland General Election'),
}
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_md-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state MD required (or --manual <file>)'); sys.exit(1)
