...
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", alaska" place-token in
...
BANDS = {
    # AK new-candidate band (165-03 generator: actual ids -20005..-20018; seqs 1-4 = unrelated KS records,
    # already imaged, skipped by the NOT EXISTS politician_images guard). Begich (-2000) falls outside
    # the band (already imaged). Guards unchanged.
    'AK': (-20099, -20001, 'Alaska'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, alaska|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_ak-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state AK required (or --manual <file>)'); sys.exit(1)
