"""
seed-co-house-headshots.py — Phase 163-03 (v2.22 Wave 3). CLONE of seed-wi-house-headshots.py, all
guards intact incl. Phase-156 hardening (dropped 'american' kw + pre-1940 historical-year reject)
and the 163-02 _FOREIGN_NATIONALITY guard. Only changes: CO band, CO state name, RESULTS_JSON, and
the localized ", colorado" place-token in _NON_PERSON_TITLE (the one token every state clone
localizes; the hardened _BAD_DISAMBIG / _HISTORICAL_YEAR / _FOREIGN_NATIONALITY / POLITICAL_KW
...
target-selection change is CO's new-candidate external_id band -80899..-80101 from 163-03.
New challengers have NO essentials.offices row, so the band query is used (not an offices-join).

NOTE (163-03): CO is NOT redistricted -- no withholding; all 8 CO races surface on the plain
'CO 2026 Statewide General' election. Incumbents (-8001..-8008) are excluded by construction
(they fall outside the band). Diana DeGette (-8001, CO-1, lost her primary) is also excluded by
construction -- she has no active race_candidates row and is outside this band either way.
...
foreign-nationality homonyms + pre-1940 historical homonyms).

Usage:
  python backend/scripts/seed-co-house-headshots.py --state CO [--dry-run]
  python backend/scripts/seed-co-house-headshots.py --manual <file>   (external_id|image_url|license)
Writes backend/scripts/_co-house-headshot-results.json.
"""
...
# CO band: -80899..-80101 (163-03 generator output, actual range -80801..-80101; excludes
# incumbents -8001..-8008 which fall far outside this band). Election name is NOT used for scoping.
BANDS = {
    'CO': (-80899, -80101, 'Colorado'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, colorado|'
...
# 163-02 Task 3 hardening (Rule 1 - bug fix, carried forward): reject any description naming a
# non-US nationality unless it also names the target US state (mirrors _desc_is_historical).
_FOREIGN_NATIONALITY = _re.compile(
    r'\b(british|scottish|english|welsh|irish|canadian|australian|indian|pakistani|nigerian|'
    r'kenyan|new zealand|french|german|italian|spanish|mexican|brazilian|japanese|chinese|'
    r'south african|zimbabwean|ghanaian)\b', _re.IGNORECASE)
def _desc_is_foreign_homonym(desc, state_name):
    if not desc:
        return False
    if state_name.lower() in desc:
        return False
    return bool(_FOREIGN_NATIONALITY.search(desc))
...
def resolve_portrait(full_name, state_name, role=''):
    ...
    if _desc_is_foreign_homonym(desc, state_name):
        return None, f'foreign-nationality-homonym-desc:"{(desc or "")[:50]}"', title
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_co-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state CO required (or --manual <file>)'); sys.exit(1)
    lo, hi, sname = BANDS[state]

    conn = psycopg2.connect(DATABASE_URL); conn.autocommit = False
    cur = conn.cursor()
    # New CO 2026 House candidates: scoped by external_id BAND ONLY (no election-name join). CO has
    # a single election (vanilla, no withholding) -- the band filter still excludes reused incumbents
    # (their external_ids like -8001 fall OUTSIDE -80101..-80899). DeGette (-8001, CO-1, lost primary)
    # has no active race_candidates row and is also excluded by construction.
    cur.execute("""
