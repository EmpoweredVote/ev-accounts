"""
seed-wa-house-headshots.py — Phase 161-04 Task 3 (v2.22 Wave 3). CLONE of seed-az-house-headshots.py
(161-02) / seed-mi-house-headshots.py (159-02), all guards intact incl. the 156 hardening (dropped
'american' kw + pre-1940 historical-year reject) and the Bouchard father/son wrong-person lesson
(first-name-mismatch rejection trusted, never overridden). Only change: WA band added (new WA
external_id band -531005..-530101 from 161-04 Task 1; incumbents -53001..-53010 fall OUTSIDE this
band and are correctly excluded).
...
Usage:
  py backend/scripts/seed-wa-house-headshots.py --state WA [--dry-run]
  py backend/scripts/seed-wa-house-headshots.py --manual <file>   (external_id|image_url|license)
Writes backend/scripts/_wa-house-headshot-results.json.
"""
...
# WA band brackets exactly the 60 new-candidate external_ids from 161-04 Task 1
# (-531005 .. -530101); incumbents -53001..-53010 fall outside this band and are excluded.
BANDS = {
    'WA': (-531005, -530101, 'Washington', 'WA 2026 Statewide General'),
}
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_wa-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state WA required (or --manual <file>)'); sys.exit(1)
    lo, hi, sname, election = BANDS[state]

    conn = psycopg2.connect(DATABASE_URL); conn.autocommit = False
    cur = conn.cursor()
    # New WA 2026 House candidates: scoped to the state's election via race_candidates (the raw
    # negative band is POLLUTED with unrelated records in general -- e.g. cross-state homonyms -- so
    # a bare band query would touch non-2026-House rows). The band filter still excludes reused
    # incumbents (their external_ids like -53001 fall OUTSIDE -530xxx/-531xxx). Phase-156 fix, carried forward.
    cur.execute("""
