"""
seed-az-house-headshots.py — Phase 161-02 Task 3 (v2.22 Wave 3). CLONE of seed-mi-house-headshots.py
(159-02), all guards intact incl. the 156 hardening (dropped 'american' kw + pre-1940 historical-year
reject) and the Bouchard father/son wrong-person lesson (first-name-mismatch rejection trusted, never
overridden). Only change: AZ band added (new AZ external_id band -40901..-40101 from 161-02 Task 1;
incumbents -4001..-4009 fall OUTSIDE this band and are correctly excluded).
...
Usage:
  py backend/scripts/seed-az-house-headshots.py --state AZ [--dry-run]
  py backend/scripts/seed-az-house-headshots.py --manual <file>   (external_id|image_url|license)
Writes backend/scripts/_az-house-headshot-results.json.
"""
...
# AZ band brackets exactly the 32 new-candidate external_ids from 161-02 Task 1
# (-40901 .. -40101); incumbents -4001..-4009 fall outside this band and are excluded.
BANDS = {
    'AZ': (-40901, -40101, 'Arizona', 'AZ 2026 Statewide General'),
}
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_az-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state AZ required (or --manual <file>)'); sys.exit(1)
    lo, hi, sname, election = BANDS[state]

    conn = psycopg2.connect(DATABASE_URL); conn.autocommit = False
    cur = conn.cursor()
    # New AZ 2026 House candidates: scoped to the state's election via race_candidates (the raw
    ...
    # incumbents (their external_ids like -4001 fall OUTSIDE -40xxx). Phase-156 fix, carried forward.
    cur.execute("""
