# Generate migration 1769: residential-zoning for the SB 5471 cohort, split 2 / 4 on SB 5184.
import json, os
...
BY = {v["memberId"]: v for v in LINK.values()}

RZ = "d4f18138-a2e0-4110-b925-7387d9d0d16d"
S = "https://lawfilesext.leg.wa.gov/biennium/2025-26/Pdf/Bills/Session%20Laws/Senate/"
SRC71 = S + "5471.SL.pdf"
SRC84 = S + "5184-S.SL.pdf"


def sp(f):
    r = json.load(open(os.path.join(C, "bills", f + ".json"), encoding="utf-8"))
    o = {}
    for s in r["sponsors"]:
        if s["id"] and s["id"] in BY and s["id"] not in o:
            o[s["id"]] = s["type"]
    return o


s71, s84 = sp("SB_5471"), sp("SB_5184")
ch4 = sorted(set(s71) & set(s84), key=lambda m: BY[m]["name"])
ch2 = sorted(set(s71) - set(s84), key=lambda m: BY[m]["name"])

B4 = ("SB 5471, which lets counties authorise middle housing on any parcel that already permits a "
      "single-family home in unincorporated urban growth areas; and of SB 5184 (Chapter law, 2025), "
      "which caps minimum parking requirements statewide — a city \"may not require more than 0.5 "
      "parking spaces\" per unit — on the finding that parking mandates \"needlessly drive up the cost "
      "of development, particularly housing\".")
B2 = ("SB 5471 (Chapter law, 2025), which lets a county authorise middle housing — duplexes and "
      "similar — on any parcel that already permits a single-family home in unincorporated urban "
      "growth areas, provided the parcel is served by water and sewer. Adoption is optional and "
      "happens by county ordinance.")

n4, n2 = len(ch4), len(ch2)
n = n4 + n2

o = []
w = o.append
w(f"""-- 1769_wa_residential_zoning_split.sql
-- {n} rows on residential-zoning, SPLIT {n2} at chair 2 and {n4} at chair 4. Bipartisan cohort:
...
for m in ch4 + ch2:
    safe = BY[m]["name"].replace("'", "''")
    w(f"""  SELECT count(*) INTO n FROM inform.politician_answers
   WHERE politician_id='{BY[m]['pid']}' AND topic_id='{RZ}';
  IF n <> 0 THEN RAISE EXCEPTION 'pre-check: {safe} already has a residential-zoning answer'; END IF;
  SELECT count(*) INTO n FROM inform.politician_context
   WHERE politician_id='{BY[m]['pid']}' AND topic_id='{RZ}';
  IF n <> 0 THEN RAISE EXCEPTION 'pre-check: {safe} already has a residential-zoning context row'; END IF;""")

w(f"""  SELECT count(*) INTO n FROM inform.compass_stances WHERE topic_id='{RZ}' AND value=2;
  IF n <> 1 THEN RAISE EXCEPTION 'pre-check: residential-zoning chair 2 not defined exactly once (%)', n; END IF;
  SELECT count(*) INTO n FROM inform.compass_stances WHERE topic_id='{RZ}' AND value=4;
  IF n <> 1 THEN RAISE EXCEPTION 'pre-check: residential-zoning chair 4 not defined exactly once (%)', n; END IF;
END $$;
""")

w("INSERT INTO inform.politician_context (politician_id, topic_id, reasoning, sources) VALUES")
vals = []
for m in ch4:
    lead = "Prime sponsor of " if s84[m] == "Primary" else "Co-sponsor of "
    vals.append(f"('{BY[m]['pid']}','{RZ}',\n $r${lead}{B4}$r$,\n ARRAY['{SRC71}','{SRC84}'])")
for m in ch2:
    lead = "Prime sponsor of " if s71[m] == "Primary" else "Co-sponsor of "
    vals.append(f"('{BY[m]['pid']}','{RZ}',\n $r${lead}{B2}$r$,\n ARRAY['{SRC71}'])")
w(",\n".join(vals) + ";\n")

w("INSERT INTO inform.politician_answers (politician_id, topic_id, value) VALUES")
w(",\n".join([f"('{BY[m]['pid']}','{RZ}', 4)" for m in ch4]
             + [f"('{BY[m]['pid']}','{RZ}', 2)" for m in ch2]) + ";\n")

p4 = ",".join("'" + BY[m]["pid"] + "'" for m in ch4)
p2 = ",".join("'" + BY[m]["pid"] + "'" for m in ch2)
...
BEGIN
  ...
  IF ans_after <> s.ans_before + {n} THEN
    RAISE EXCEPTION 'guard 1: answers % -> %, expected +{n}', s.ans_before, ans_after; END IF;
  IF ctx_after <> s.ctx_before + {n} THEN
    RAISE EXCEPTION 'guard 1: context % -> %, expected +{n}', s.ctx_before, ctx_after; END IF;
END $$;
...
BEGIN
  SELECT count(*) INTO bad
   ...
   WHERE a.topic_id='{RZ}' AND a.politician_id IN ({p4})
     AND (a.value <> 4 OR c.reasoning !~ 'may not require more than' OR NOT ('{SRC84}' = ANY(c.sources)));
  ...
  SELECT count(*) INTO bad
    FROM inform.politician_answers a
    JOIN inform.politician_context c ON c.politician_id=a.politician_id AND c.topic_id=a.topic_id
   WHERE a.topic_id='{RZ}' AND a.politician_id IN ({p2})
     AND (a.value <> 2 OR c.reasoning !~ 'Adoption is optional' OR NOT ('{SRC71}' = ANY(c.sources)));
  ...
  SELECT count(*) INTO c4 FROM inform.politician_answers WHERE topic_id='{RZ}' AND value=4 AND politician_id IN ({p4});
  SELECT count(*) INTO c2 FROM inform.politician_answers WHERE topic_id='{RZ}' AND value=2 AND politician_id IN ({p2});
  IF c4 <> {n4} THEN RAISE EXCEPTION 'guard 2: chair-4 count is %, expected {n4}', c4; END IF;
  IF c2 <> {n2} THEN RAISE EXCEPTION 'guard 2: chair-2 count is %, expected {n2}', c2; END IF;
END $$;
...
BEGIN
  ...
  RAISE NOTICE 'residential-zoning: {n2} at chair 2, {n4} at chair 4; ORPHAN_CONTEXT still 50';
END $$;

COMMIT;
""")

dest = r"C:\EV-Accounts\backend\migrations\1769_wa_residential_zoning_split.sql"
open(dest, "w", encoding="utf-8", newline="\n").write("\n".join(o))
print(f"wrote {dest}")
print(f"rows {n}: chair2={n2} chair4={n4}")
for m in ch2: print('   ch2', BY[m]['name'], BY[m]['party'])
for m in ch4: print('   ch4', BY[m]['name'], BY[m]['party'])
