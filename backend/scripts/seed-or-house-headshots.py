"""
seed-or-house-headshots.py — Phase 164-03 (v2.22 Wave 3). CLONE of seed-ks-house-headshots.py, all
guards intact incl. Phase-156 hardening (dropped 'american' kw + pre-1940 historical-year reject).
Only changes: OR band, OR state name, RESULTS_JSON, and the localized ", oregon" place-token in
...
BANDS = {
    # OR new-candidate band (164-03 generator: actual range -410601..-410114). Upper bound is
    # -410114 (OR-1 seq 14) to EXCLUDE the 4 OR LOCAL officials at -410110..-410113 (the parallel
    # 177/178 Hillsboro/Tigard session's rows) — we never touch those. OR US-House incumbents
    # (-4102001..-4102006) are 7-digit ids far outside this 6-digit band (already imaged). Guards unchanged.
    'OR': (-410699, -410114, 'Oregon'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, oregon|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_or-house-headshot-results.json')
