...
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", south dakota" place-token in
...
BANDS = {
    # SD new-candidate band (165-08: -460001 Gronli). Jackley reuses the imaged v2.18 pid; Johnson outside band.
    'SD': (-460099, -460001, 'South Dakota'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, south dakota|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_sd-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state SD required (or --manual <file>)'); sys.exit(1)
