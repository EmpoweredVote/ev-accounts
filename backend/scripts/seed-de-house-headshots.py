...
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", delaware" place-token in
...
BANDS = {
    # DE band LOWER-BOUNDED at -100048 (safe_start_seq=48): seqs 1-47 = 26 unrelated legacy Wave-1 records that MUST NOT be touched (Pitfall 6).
    'DE': (-100099, -100048, 'Delaware'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, delaware|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_de-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state DE required (or --manual <file>)'); sys.exit(1)
