...
Only changes: MS band, MS state name, RESULTS_JSON, and the localized ", nevada" place-token in
...
BANDS = {
    # NV new-candidate band (165-01: -320174/-320175/-320251/-320301/-320479/-320480). NV incumbents
    # (-32001..-32004) are 5-digit ids outside this band (already imaged). Guards unchanged.
    'NV': (-320499, -320101, 'Nevada'),
}
...
_NON_PERSON_TITLE = _re.compile(
    r'(election|congressional district|gubernatorial|ballot|primary|\bcity\b|, nevada|'
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_nv-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state NV required (or --manual <file>)'); sys.exit(1)
