"""
seed-tx-ny-house-headshots.py — Phase 150 (v2.20 2026 US House Candidate Coverage) Wave 3/4,
plans 150-05 (TX) + 150-06 (NY).

CLONE of seed-ca-house-headshots.py (the validated Phase-149 pipeline) — ALL guards intact. The ONLY
change is target-selection: parameterized by `--state TX|NY` to select the new-candidate external_id
band (TX -4819999..-4810000 from 150-03; NY -3619999..-3610000 from 150-04). New challengers have NO
essentials.offices row, so the band query is used (not an offices-join). The SAME script serves both
150-05 and 150-06.
...
Usage:
  python backend/scripts/seed-tx-ny-house-headshots.py --state TX [--dry-run]
  python backend/scripts/seed-tx-ny-house-headshots.py --state NY
  python backend/scripts/seed-tx-ny-house-headshots.py --manual <file>   (external_id|image_url|license)
Writes backend/scripts/_tx-ny-house-headshot-results.json.
"""
...
BANDS = {
    'TX': (-4819999, -4810000, 'Texas'),
    'NY': (-3619999, -3610000, 'New York'),
    'FL': (-1229999, -1210000, 'Florida'),
}

# Phase 151 (151-04): ONLY the 17 FL independent/NPA new candidates are in scope (D-01
# records-now/stances-at-153) — NOT the full FL new-candidate band (which includes the 138
# partisan primary candidates deferred to Phase 153). external_ids from 151-03-SUMMARY.
FL_INDEP_EXTERNAL_IDS = [
    -1210104,  # FL-1  Tyler Davis
    -1210305,  # FL-3  Mike Klein
    -1210405,  # FL-4  Todd Schaefer
    -1210609,  # FL-6  Andrew Parrott
    -1210610,  # FL-6  Alec Pavlik
    -1211203,  # FL-12 Branden Scrivener
    -1211304,  # FL-13 Tony D'Arrigo
    -1211609,  # FL-16 Mark Davis
    -1211703,  # FL-17 Michael Quirk
    -1211802,  # FL-18 Deva Simmons
    -1211914,  # FL-19 Seth Haskins
    -1212009,  # FL-20 Kedner MaximeDe
    -1212103,  # FL-21 Alexander Cooke
    -1212409,  # FL-24 Andy Daro
    -1212410,  # FL-24 Patricia Gonzalez
    -1212602,  # FL-26 Deborah Ann Meidinger Hosey
    -1212802,  # FL-28 Eddy Rojas
]
...
RESULTS_JSON = os.path.join(os.path.dirname(__file__), '_tx-ny-house-headshot-results.json')
...
def main():
    ...
    if state not in BANDS:
        print('ERROR: --state TX|NY|FL required (or --manual <file>)'); sys.exit(1)
    lo, hi, sname = BANDS[state]

    conn = psycopg2.connect(DATABASE_URL); conn.autocommit = False
    cur = conn.cursor()
    if state == 'FL':
        # Phase 151: ONLY the 17 independent/NPA new candidates (explicit list, NOT the full band —
        # the 138 partisan candidates are deferred to Phase 153 per D-01).
        cur.execute("""
        ...
        # New TX/NY 2026 House challengers (band per --state). NO essentials.offices row -> band query.
        cur.execute("""
            SELECT p.id, p.external_id, p.full_name
            FROM essentials.politicians p
