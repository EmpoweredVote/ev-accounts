 ...
 * 6 target Multnomah County school districts by GEOID, and inserts
 * G5420 geofence_boundaries rows into essentials.geofence_boundaries.
 *
 * Each district is stored with:
 *   geo_id  = GEOID value directly (e.g. '4110040') — NOT a slug like LAUSD
 *   mtfcc   = 'G5420'
 ...
 *   npx tsx scripts/load-or-school-boundaries.ts --dry-run
 *   npx tsx scripts/load-or-school-boundaries.ts
 */
...
// All 6 GEOIDs verified via TIGERweb REST API (layer 14) 2026-06-01.
...
async function main() {
  ...
  try {
    ...
    // Step 5: Assert all 6 GEOIDs were found
  ...
  console.log('\n[load-or-school-boundaries] Done.');
}
