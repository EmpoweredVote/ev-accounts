...
//
// Adjusts a user's verification_rating by a delta for Yellow quest grading.
// Simpler than confirm-stance: no gems, no politician stance upsert.
// VR is clamped to [0, 100]. Sets vq_hold_until when VR hits 0.
//
// Idempotent: duplicate idempotency_key returns 200 with replayed: true
// and no side effects.
// ---------------------------------------------------------------------------
...
router.post(
  ...
  requireGemServiceKey,
  async (req: Request, res: Response): Promise<void> => {
    const parsed = AdjustVrBodySchema.safeParse(req.body);
    if (!parsed.success) {
      res.status(422).json({
        error: 'VALIDATION_ERROR',
        issues: parsed.error.issues,
    ...
    try {
      const result = await adjustVerificationRating({
        userId:         body.user_id,
        delta:          body.delta,
        idempotencyKey: body.idempotency_key,
        reason:         body.reason,
      ...
      console.error('[POST /vq/adjust-vr] error:', err);
      res.status(500).json({ error: 'INTERNAL_ERROR' });
    }
  }
);
