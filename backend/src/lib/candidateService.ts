...
/**
 * Fetch all ACTIVE candidates representing a given ZIP code.
 *
 * CRITICAL: is_active = true filter is enforced — Essentials NEVER returns
 * inactive (demoted) candidates. This is intentional: demoted candidates no
 * longer hold the office and should not appear in local discovery.
 *
 * Uses supabaseAdmin — the RLS "public read active" policy would allow reads,
 * but compass_responses reads elsewhere in this service require supabaseAdmin,
 * and using a single client throughout keeps the pattern consistent.
 *
 * Cache TTL: 900s (15 min) per ZIP code.
 */
export async function getCandidatesByZip(zip: string): Promise<EssentialsCandidate[]> {
  const cacheKey = `candidates:zip:${zip}`;

  // Cache check
  const cached = await cache.get<EssentialsCandidate[]>(cacheKey);
  if (cached !== null) return cached;
  ...
  // Map through explicit whitelist serializer — NEVER spread rows
  const candidates: EssentialsCandidate[] = (data ?? []).map((row) => {
    const names = splitLegalName(row.legal_name as string);
    return {
  ...
  await cache.set(cacheKey, candidates, 900);
  return candidates;
}
