...
export class SessionStorageFactory {
  ...
  /**
   * Initialize storage backend based on TRIVIA_REDIS_URL environment variable
   * Falls back to MemoryStorage if Redis unavailable.
   *
   * NOTE (engine fold-in, Phase 1): reads TRIVIA_REDIS_URL, NOT the engine's REDIS_URL.
   * The engine's REDIS_URL is an Upstash REST (https://) endpoint used by @upstash/redis
   * over HTTP; node-redis here needs a TCP redis:// / rediss:// URL and cannot use it.
   * A distinct var avoids a guaranteed mis-connect. Unset => MemoryStorage (functional on
   * a single instance; sessions reset on redeploy). Set it to a TCP Redis for durability.
   */
  async initialize(): Promise<void> {
    const redisUrl = process.env.TRIVIA_REDIS_URL;

    // No TRIVIA_REDIS_URL - use in-memory storage
    if (!redisUrl) {
      console.log('⚠️  TRIVIA_REDIS_URL not set, using in-memory storage');
