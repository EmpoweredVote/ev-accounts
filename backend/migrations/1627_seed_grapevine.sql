-- 1627: seed City of Grapevine, Texas, US - City Council
--
-- 7 seats: 7 occupied, 0 seeded VACANT.
--
-- Mayor + six at-large Places, three-year terms. Mayor Pro Tem is not a title.
--
...
BEGIN
  IF (SELECT count(*) FROM essentials.chambers c JOIN essentials.governments g ON g.id=c.government_id
       WHERE g.name='City of Grapevine, Texas, US' AND c.name='City Council') > 0 THEN
    RAISE EXCEPTION 'Migration 1627 already applied';
  END IF;
  IF (SELECT count(*) FROM essentials.geofence_boundaries WHERE geo_id='4830644' AND mtfcc='G4110') = 0 THEN
    RAISE EXCEPTION 'Pre-flight FAILED: geofence 4830644/G4110 not found';
  END IF;
END $$;

INSERT INTO essentials.governments (id, name, type, state, city, geo_id)
SELECT gen_random_uuid(), 'City of Grapevine, Texas, US', 'LOCAL', 'TX', 'Grapevine', '4830644'
WHERE NOT EXISTS (SELECT 1 FROM essentials.governments WHERE name='City of Grapevine, Texas, US');

INSERT INTO essentials.districts (id, district_type, state, geo_id, label, mtfcc, ocd_id, government_id)
SELECT gen_random_uuid(), 'LOCAL', 'tx', '4830644', 'City of Grapevine', 'G4110',
       'ocd-division/country:us/state:tx/place:grapevine',
       (SELECT id FROM essentials.governments WHERE name='City of Grapevine, Texas, US')
WHERE NOT EXISTS (SELECT 1 FROM essentials.districts
  WHERE geo_id='4830644' AND district_type='LOCAL' AND state='tx');

DO $$
BEGIN
  IF (SELECT count(*) FROM essentials.districts
       WHERE geo_id='4830644' AND district_type='LOCAL' AND state='tx') <> 1 THEN
    RAISE EXCEPTION 'expected exactly 1 LOCAL district for 4830644';
  END IF;
END $$;

INSERT INTO essentials.chambers (id, name, name_formal, government_id, official_count)
SELECT gen_random_uuid(), 'City Council', 'Grapevine City Council',
       (SELECT id FROM essentials.governments WHERE name='City of Grapevine, Texas, US'), 7
WHERE NOT EXISTS (SELECT 1 FROM essentials.chambers c JOIN essentials.governments gg ON gg.id=c.government_id
  WHERE gg.name='City of Grapevine, Texas, US' AND c.name='City Council');
...
WITH seats(title, full_name, first_name, last_name) AS (VALUES
  ('Mayor', 'William D. Tate', 'William', 'Tate'),
  ('Council Member Place 1', 'Paul Slechta', 'Paul', 'Slechta'),
  ('Council Member Place 2', 'Sharron Rogers', 'Sharron', 'Rogers'),
  ('Council Member Place 3', 'Leon Leal', 'Leon', 'Leal'),
  ('Council Member Place 4', 'Sean Shope', 'Sean', 'Shope'),
  ('Council Member Place 5', 'Chris Coy', 'Chris', 'Coy'),
  ('Council Member Place 6', 'Duff O''Dell', 'Duff', 'O''Dell')
), ofc AS (
  INSERT INTO essentials.offices
    (id, chamber_id, district_id, title, representing_state, representing_city, seats,
     is_appointed_position, is_vacant, role_canonical)
  SELECT gen_random_uuid(), (SELECT id FROM essentials.chambers WHERE name='City Council' AND government_id=(SELECT id FROM essentials.governments WHERE name='City of Grapevine, Texas, US')), (SELECT id FROM essentials.districts WHERE geo_id='4830644' AND district_type='LOCAL' AND state='tx'), s.title, 'TX', 'Grapevine', 1, false, false, NULL
    FROM seats s
  RETURNING id, title
), pol AS (
...
BEGIN
  SELECT (SELECT id FROM essentials.chambers WHERE name='City Council' AND government_id=(SELECT id FROM essentials.governments WHERE name='City of Grapevine, Texas, US')) INTO v_ch;
