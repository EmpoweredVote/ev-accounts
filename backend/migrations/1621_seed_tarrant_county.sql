-- 1621: seed Tarrant County, Texas, US - Commissioners Court
--
-- 5 seats: 5 occupied, 0 seeded VACANT.
--
-- New government row. COUNTY district on 48439 already exists (G4020) and is reused.
--
...
BEGIN
  IF (SELECT count(*) FROM essentials.chambers c
       ...
       WHERE g.name = 'Tarrant County, Texas, US' AND c.name = 'Commissioners Court') > 0 THEN
    RAISE EXCEPTION 'Migration 1621 already applied - Tarrant County, Texas, US / Commissioners Court exists';
  END IF;
END $$;
...
BEGIN
  IF (SELECT count(*) FROM essentials.geofence_boundaries
       WHERE geo_id = '48439' AND mtfcc = 'G4020') = 0 THEN
    RAISE EXCEPTION 'Pre-flight FAILED: geofence 48439/G4020 not found';
  END IF;
END $$;

-- Government row (new county).
INSERT INTO essentials.governments (id, name, type, state, city, geo_id)
SELECT gen_random_uuid(), 'Tarrant County, Texas, US', 'County', 'TX', NULL, '48439'
WHERE NOT EXISTS (SELECT 1 FROM essentials.governments WHERE name = 'Tarrant County, Texas, US');
...
BEGIN
  IF (SELECT count(*) FROM essentials.districts
       WHERE geo_id = '48439' AND district_type = 'COUNTY' AND state = 'tx') <> 1 THEN
    RAISE EXCEPTION 'expected exactly 1 COUNTY district for 48439';
  END IF;
END $$;

-- Chamber. slug is GENERATED ALWAYS - never list it.
INSERT INTO essentials.chambers (id, name, name_formal, government_id, official_count)
SELECT gen_random_uuid(), 'Commissioners Court', 'Tarrant County Commissioners Court',
       (SELECT id FROM essentials.governments WHERE name = 'Tarrant County, Texas, US'), 5
WHERE NOT EXISTS (
  SELECT 1 FROM essentials.chambers c JOIN essentials.governments gg ON gg.id = c.government_id
   WHERE gg.name = 'Tarrant County, Texas, US' AND c.name = 'Commissioners Court');
...
WITH ofc AS (
  ...
  SELECT gen_random_uuid(), (SELECT id FROM essentials.chambers WHERE name='Commissioners Court' AND government_id=(SELECT id FROM essentials.governments WHERE name='Tarrant County, Texas, US')), (SELECT id FROM essentials.districts WHERE geo_id='48439' AND district_type='COUNTY' AND state='tx'),
  ...
  SELECT gen_random_uuid(), 'Tim O''Hare', 'Tim', 'O''Hare', NULL,
...
WITH ofc AS (
  ...
  INSERT INTO essentials.politicians
    (id, full_name, first_name, last_name, party, is_active, is_appointed, is_vacant,
     is_incumbent, data_source, office_id)
  SELECT gen_random_uuid(), 'Roderick Miles Jr', 'Roderick', 'Miles', NULL,
         true, false, false, true, 'Tarrant seed 2026-08-08; see backend/data/seed-tarrant-2026/ROSTERS.md', ofc.id
    FROM ofc
  RETURNING id, office_id
)
...
WITH ofc AS (
  ...
  INSERT INTO essentials.politicians
    (id, full_name, first_name, last_name, party, is_active, is_appointed, is_vacant,
     is_incumbent, data_source, office_id)
  SELECT gen_random_uuid(), 'Alisa Simmons', 'Alisa', 'Simmons', NULL,
         true, false, false, true, 'Tarrant seed 2026-08-08; see backend/data/seed-tarrant-2026/ROSTERS.md', ofc.id
    FROM ofc
  RETURNING id, office_id
)
...
WITH ofc AS (
  ...
  INSERT INTO essentials.politicians
    (id, full_name, first_name, last_name, party, is_active, is_appointed, is_vacant,
     is_incumbent, data_source, office_id)
  SELECT gen_random_uuid(), 'Matt Krause', 'Matt', 'Krause', NULL,
         true, false, false, true, 'Tarrant seed 2026-08-08; see backend/data/seed-tarrant-2026/ROSTERS.md', ofc.id
    FROM ofc
  RETURNING id, office_id
)
...
WITH ofc AS (
  ...
  INSERT INTO essentials.politicians
    (id, full_name, first_name, last_name, party, is_active, is_appointed, is_vacant,
     is_incumbent, data_source, office_id)
  SELECT gen_random_uuid(), 'Manny Ramirez', 'Manny', 'Ramirez', NULL,
         true, false, false, true, 'Tarrant seed 2026-08-08; see backend/data/seed-tarrant-2026/ROSTERS.md', ofc.id
    FROM ofc
  RETURNING id, office_id
)
...
DECLARE v_off int; v_fill int; v_vac int; v_term int; v_orphan int;
  v_chamber uuid;
BEGIN
  SELECT (SELECT id FROM essentials.chambers WHERE name='Commissioners Court' AND government_id=(SELECT id FROM essentials.governments WHERE name='Tarrant County, Texas, US')) INTO v_chamber;
