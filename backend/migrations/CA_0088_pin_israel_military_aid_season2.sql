-- CA_0088_pin_israel_military_aid_season2.sql
-- Author: Chris Andrews (CA_ namespace, Andrews' slot)
-- Runs AFTER CA_0087 (the israel-military-aid topic must exist). Renumber to the next free CA_#### if taken.
--
-- =============================================================================
-- CA_0088: Pin "Israel Military Aid" into Season 2 (Season 2 is a DRAFT; NOT opened here)
-- =============================================================================
-- Created 2026-08-31 with Chris Andrews (actor 854fbc06-40fc-458d-b523-20ef8e5ad1b2).
--
-- WHAT: adds the staged topic israel-military-aid (CA_0087) to Season 2's question set via
--   inform.admin_season_add_topic, which pins the topic's current published v1 revision.
--   This is the compass's second foreign-policy topic (sibling: Ukraine Support).
--
-- WHY SAFE NOW: Season 2 (86d893a1-…) is a DRAFT. A season serves its pinned questions only
--   when OPEN, so this changes nothing a voter sees today. The topic stays is_live=false and
--   is NOT promoted (the season-gated promoted view lists only the OPEN season's topics). It
--   goes live when Season 2 opens (inform.admin_open_season) — a separate, later step. Season 1
--   (open) is untouched.
--
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM inform.seasons WHERE id = v_season AND status = 'draft') THEN
    RAISE EXCEPTION 'CA_0088 verify: Season 2 is no longer a draft';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'CA_0088 verify: % is not pinned to Season 2 at its current published revision', v_key;
  END IF;
  ...
  IF (SELECT is_live FROM inform.compass_topics WHERE id = v_topic) THEN
    RAISE EXCEPTION 'CA_0088 verify: % is_live=true (must stay staged until Season 2 opens)', v_key;
  END IF;
  IF EXISTS (SELECT 1 FROM inform.compass_topics_promoted WHERE id = v_topic) THEN
    RAISE EXCEPTION 'CA_0088 verify: % is promoted before Season 2 opened', v_key;
  END IF;

  RAISE NOTICE 'CA_0088 post-verify OK — israel-military-aid pinned to Season 2 (draft), still staged & unpromoted. It goes live when Season 2 opens.';
END $$;
