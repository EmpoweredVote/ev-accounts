...
--
-- ✅ APPLIED TO PROD 2026-08-21. Authored in the on-the-record repo
-- (backend-migrations-draft/) and filed here on application. Generated 2026-08-17.
--
-- Applied as `postgres` (the table owner) over the port-5432 session pooler. Dry-run first with
-- COMMIT swapped for ROLLBACK per house style: the post-verify gate passed inside the transaction
-- and the rollback was confirmed to have reverted (role still NOT NULL, 26 rows) before the real
-- run. After applying: role is nullable, the column COMMENT is set, local_people_role_check is
-- untouched, and the role distribution is unchanged at candidate=25 / moderator=1 — no backfill.
-- The publish path was then proved end-to-end: the exact INSERT ... ON CONFLICT that
-- _upsert_local_people issues, with role NULL, was accepted (and rolled back).
--
-- Re-running is a no-op: DROP NOT NULL on an already-nullable column changes nothing and the gate
-- still passes. To revert, `ALTER COLUMN role SET NOT NULL` — which works only while no NULL role
-- has been written, i.e. before the next publish of a role-less local person.
--
