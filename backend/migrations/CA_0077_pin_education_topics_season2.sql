-- CA_0077_pin_education_topics_season2.sql
...
--
-- =============================================================================
-- CA_0077: Pin the 8 education topics into Season 2 (Season 2 is a DRAFT; NOT opened here)
-- =============================================================================
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM inform.seasons WHERE id = v_season AND status = 'draft') THEN
    RAISE EXCEPTION 'CA_0077: Season 2 is not a draft — question set is frozen, refusing to pin';
  END IF;
  ...
  LOOP
    ...
    IF v_topic IS NULL THEN
      RAISE EXCEPTION 'CA_0077: topic % is missing — apply CA_0075 first', v_key;
    END IF;

    IF EXISTS (SELECT 1 FROM inform.season_questions WHERE season_id = v_season AND topic_id = v_topic) THEN
      RAISE NOTICE 'CA_0077: % already in Season 2 — skip', v_key;
    ELSE
      PERFORM inform.admin_season_add_topic(v_season, v_topic, v_actor);
      v_added := v_added + 1;
      RAISE NOTICE 'CA_0077: pinned % to Season 2', v_key;
    END IF;
  END LOOP;

  RAISE NOTICE 'CA_0077: % of 8 education topics newly pinned to Season 2', v_added;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM inform.seasons WHERE id = v_season AND status = 'draft') THEN
    RAISE EXCEPTION 'CA_0077 verify: Season 2 is no longer a draft';
  END IF;
  ...
  LOOP
    ...
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION 'CA_0077 verify: % is not pinned to Season 2 at its current published revision', v_key;
    END IF;
    ...
    IF (SELECT is_live FROM inform.compass_topics WHERE id = v_topic) THEN
      RAISE EXCEPTION 'CA_0077 verify: % is_live=true (must stay staged until Season 2 opens)', v_key;
    END IF;
    IF EXISTS (SELECT 1 FROM inform.compass_topics_promoted WHERE id = v_topic) THEN
      RAISE EXCEPTION 'CA_0077 verify: % is promoted before Season 2 opened', v_key;
    END IF;
  END LOOP;
  ...
  IF v_n <> 8 THEN
    RAISE EXCEPTION 'CA_0077 verify: expected 8 education topics in Season 2, found %', v_n;
  END IF;

  RAISE NOTICE 'CA_0077 post-verify OK — 8 education topics pinned to Season 2 (draft), still staged & unpromoted. They go live when Season 2 opens.';
END $$;
