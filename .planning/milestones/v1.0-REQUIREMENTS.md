...
- [x] **ADMN-01**: Admin can create invite codes, revoke active codes, view the full invite chain, and trace the invite tree for any account
- [x] **ADMN-02**: Admin can view account details including `tolerance_rating` and `legal_name` (internal-only fields — admin context only, logged to audit log on access)
- [x] **ADMN-03**: Admin can approve, suspend, and reinstate accounts; can set `account_standing`
- [x] **ADMN-04**: Admin can manage pilot cohort enrollment (assign cohort, view cohort membership)
- [x] **ADMN-05**: Every admin action is appended to `admin_audit_log` with actor, action, target, and timestamp — no admin action is unlogged
- [x] **ADMN-06**: Admin authentication uses a separate `admin_users` table keyed by `user_id`; being an authenticated user is not sufficient to access admin routes

### Calibration Lapse Enforcement (CRON)

- [x] **CRON-01**: Daily scheduled job (2am UTC) identifies Empowered users who have not calibrated new live topics within the 30-day grace window
- [x] **CRON-02**: At day 25, a warning notification is dispatched to at-risk Empowered accounts (delivery channel TBD — placeholder event emitted; wired to email or in-app in later phase)
- [x] **CRON-03**: At day 30, automatic demotion executes via `execute_demotion` RPC for each lapsed user; user notified with specific topic(s) requiring calibration and re-empowerment path
- [x] **CRON-04**: Job execution is idempotent — date-keyed `calibration_lapse_runs` table prevents double execution across server restarts or deploys
...
| ADMN-01 | Phase 7 — Admin Tool and Calibration Cron | Complete |
| ADMN-02 | Phase 7 — Admin Tool and Calibration Cron | Complete |
| ADMN-03 | Phase 7 — Admin Tool and Calibration Cron | Complete |
| ADMN-04 | Phase 7 — Admin Tool and Calibration Cron | Complete |
| ADMN-05 | Phase 7 — Admin Tool and Calibration Cron | Complete |
| ADMN-06 | Phase 7 — Admin Tool and Calibration Cron | Complete |
| CRON-01 | Phase 7 — Admin Tool and Calibration Cron | Complete |
| CRON-02 | Phase 7 — Admin Tool and Calibration Cron | Complete |
| CRON-03 | Phase 7 — Admin Tool and Calibration Cron | Complete |
| CRON-04 | Phase 7 — Admin Tool and Calibration Cron | Complete |
...
**Shipped:** 50 of 52 v1 requirements
**Adjusted:** CIVIC-02 (gem reserve cap) — deferred by design per CONTEXT.md; ledger otherwise complete
**Deferred:** COMP-05 (user-to-user compass compare) — infrastructure in place, politician compare only for v1
