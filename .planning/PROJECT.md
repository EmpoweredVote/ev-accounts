...
**v2.22 Phase 161 complete 2026-07-04 (anchor seeding phase)** — the 2026 US House field for the four largest remaining delegations is live on `/elections`: **WA 10 + AZ 9 + TN 9 + MA 9 = 37 districts**, **183 new candidate records** (AZ 32 / WA 60 / TN 73 / MA 18), federal-24 chairs-not-polarity stances (**0 unsourced**; 59 documented whole-record honest-skips reflecting thin post-redistricting minor fields), headshots (167 documented skips). WA/AZ/TN authored elections+races first; MA wired candidates-only onto its 9 pre-existing races (Clark/Pressley un-duplicated). **TN mid-cycle redistricting handled by D-01b**: 5 severe-shift districts (geo_id 4704/05/06/08/09) are SEEDED BUT WITHHELD from `/elections` via a past-dated "Polygon Pending" election (reps feed / `essentials.offices` untouched — self-heals in Phase 164.1); the other 4 TN + all WA/AZ/MA surface. AZ roster reconciliation (mig 1204) set 5 ballot-ineligible candidates to `withdrawn`. Gate `161-verify.sql` (11/11) + `161-coordinate-smoke.ts` (5/5 incl. severe-TN zero-race negative) pass; gsd-verifier 4/4 independently re-run. Migrations 1187/1188 (AZ), 1189/1190 (WA), 1196/1197 (TN), 1202 (MA), 1204 (AZ recon) — number collisions with the parallel OR session documented + non-blocking. Pure-data. **Next: Phase 162 (IN+MD+MN+MO).** Execution note: stance CSVs are gitignored scratch — prod is the durable store; incremental per-batch prod pushes made the phase resilient to repeated Fable-5/session-limit interruptions.
...
---
*Last updated: 2026-07-03 — v2.22 Phase 160 complete: 178-district field table resolved (88 decided / 90 late-primary, seeding_phase 161-165), incumbent map + collision audit (19 districts incl. at-large cd=0 bands) + 29-race preexistence baseline; 5 mid-cycle redistricting flags (TN/MO/AL/LA/UT — UT court-ordered re-key required). v2.22 2026 US House Candidate Coverage (Wave 3 — National Completion) started. Scope: the 38 remaining states = 178 districts (all delegations ≤ 10), completing all 435 districts; primary-decided states seed final fields, late-primary states seed full provisional fields (FL/MI/VA pattern) with date-gated culls. Wave 3 phases start at 160 (159 preserved for the v2.21 date-gated tail; 177/178 reserved by a parallel session). v2.21 Waves 1-2 done; its 159-05/06 cull+gate is date-gated ≥ 2026-08-05.*
