...
| 2 | No response includes tolerance_rating -- confirmed by integration test asserting field absence not merely null | VERIFIED | grep tolerance_rating in candidateService.ts: zero matches; CandidateProfile and EssentialsCandidate types have no tolerance_rating field; candidates.test.ts line 154 static CI assertion; DB-dependent test line 222 asserts field structurally absent from live response |
| 3 | A slug belonging to a demoted inactive Empowered account returns a consistent inactive state response and cannot be claimed by a new user | VERIFIED | getCandidateBySlug has NO is_active filter -- supabaseAdmin bypasses RLS; response includes active: data.is_active and demoted_at; named UNIQUE constraint empowered_profiles_candidate_page_slug_unique in migration 025; getCandidatesByZip enforces is_active = true |
...
| backend/src/lib/candidateService.ts | 4 functions exported; no tolerance_rating; no DB row spreading | VERIFIED | 319 lines; all 4 functions exported; tolerance_rating absent (grep: zero matches); no row spread (grep: zero matches); supabaseAdmin and cache imported and used |
| backend/src/routes/candidates.ts | GET /:slug/answers before GET /:slug; optionalAuth; no supabaseAdmin | VERIFIED | 103 lines; /:slug/answers at line 31 before /:slug at line 85; optionalAuth on both handlers; supabaseAdmin: zero matches |
| backend/src/routes/essentialsCandidates.ts | GET /:zip; ZIP regex; optionalAuth; no supabaseAdmin | VERIFIED | 57 lines; ZIP_REGEX accepts 5-digit and ZIP+4; normalize to 5-digit via slice(0,5); optionalAuth; supabaseAdmin: zero matches |
...
| tests/integration/candidates.test.ts | 18 CI-safe tests passing; 9 DB-dependent skipped; tolerance_rating absence assertion | VERIFIED | 343 lines; vitest run: 18 passed / 9 skipped; static CI assertion at line 154; it.skip DB tests assert field structurally absent from live response |
...
| backend/src/lib/candidateService.ts | cache | import from ./cache.js line 23 | WIRED | cache.get and cache.set called in getCandidateBySlug and getCandidatesByZip with 900s TTL |
...
| tests/integration/candidates.test.ts | tolerance_rating absence | static file content assertion line 154 | WIRED | expect(serviceSource).not.toContain assertion runs without live DB |
...
No anti-patterns detected in Phase 8 files. No TODOs, FIXMEs, placeholder returns, empty handlers, or console.log-only implementations found.
...
**Test:** With Phase 8 migration applied and a demoted candidate (is_active = false, demoted_at set), call GET /api/candidates/{slug} without an Authorization header
**Expected:** HTTP 200 with active: false, demoted_at as ISO date string, first_name and last_name populated; tolerance_rating structurally absent from response body
**Why human:** The 9 DB-dependent tests are it.skip per decision [04-03] -- require live Supabase with migration 025 applied and seeded candidate data
