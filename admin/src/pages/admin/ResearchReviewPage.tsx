...
export function ResearchReviewPage() {
  ...
  async function handleApprove() {
    ...
    setError(null);
    try {
      await apiFetch(`/admin/research-review/${id}/resolve`, { method: 'POST' });
  ...
  const verifiedSources = row.evidence.filter((e) => e.snippets.some((s) => s.verdict === 'verified'));
  const failedSources = row.evidence.filter((e) => !e.snippets.some((s) => s.verdict === 'verified'));
    ...
    <div className="max-w-2xl">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-6 space-y-5">
        ...
        <div className="flex flex-wrap gap-3 text-sm text-gray-500 dark:text-gray-400 pb-4 border-b border-gray-100 dark:border-gray-800">
          ...
          <span>{row.verifiedSourceCount}/{row.threshold} sources verified</span>
          {row.reResearchAttempted && <span className="text-yellow-600 dark:text-yellow-400">Re-research attempted</span>}
        </div>
        ...
        {row.evidence.length > 0 && (
          <div>
            ...
            <div className="space-y-3">
              {verifiedSources.length > 0 && verifiedSources.map((src, i) => (
              ...
              {failedSources.length > 0 && (
                <details className="group">
                  <summary className="cursor-pointer text-xs text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300">
                    {failedSources.length} unverified source{failedSources.length !== 1 ? 's' : ''}
                  </summary>
...
function SourceBlock({ src }: { src: EvidenceSource }) {
  const hasVerified = src.snippets.some((s) => s.verdict === 'verified');
  return (
  ...
  return (
    <div className="text-xs space-y-0.5">
      <span className={`font-medium ${color}`}>
        {snip.verdict === 'verified' ? '✓ verified' : snip.verdict === 'url_broken' ? '✗ broken' : '– not found'}
        {snip.verdict === 'url_broken' && snip.reason ? `: ${snip.reason}` : ''}
      </span>
      {snip.snippet && (
